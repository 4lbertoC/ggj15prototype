...
public class Game
{
    ...
    public static void VictorySequence(List<Guy> nonPedroes, List<Guy> savedGuys)
    {
        ...
        bool finalVictory = (Main.gameLevel >= Main.MAX_GAME_LEVEL);
        // Question balloon should be left of the answer balloon:
        if (juan.transform.position.x < gonzalo.transform.position.x)
        {
            juan.ShowAndNowMessage(finalVictory ? gonzalo : null, savedGuys);
        }
        else {
            gonzalo.ShowAndNowMessage(finalVictory ? juan : null, savedGuys);
        }
    }
}
