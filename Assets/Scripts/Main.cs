...
public class Main : MonoBehaviour
{
    ...
    public void Play(int level)
    {
        ...
        List<Vector3> pickedPositions = new List<Vector3>(positions);
        while (pickedPositions.Count > Mathf.Min(gameLevel, positions.Count))
        {
            pickedPositions.RemoveAt(Random.Range(0, pickedPositions.Count));
        }

        for (int i = 0; i < pickedPositions.Count; i++)
        {
            GameObject guyGO = (GameObject)Instantiate(guyPrefab, pickedPositions[i], defaultRotation);
            Instantiate(housePrefab, pickedPositions[i], Quaternion.identity);
