...
public class GameState
{
    ...
    public void ProceedByRunning(Guy runner)
    {
        ...
        if (runner == currentPedro)
        {
            ...
            {
                guy.GetComponent<Collider>().enabled = false;
            }
            foreach (Guy guy in currentGuys)
            {
        ...
        if (AudioPlayer.GetInstance() != null)
        {
            switch (phase)
            {
                case GamePhase.Ready:
                    AudioPlayer.GetInstance().PlayMusic(1);
                    break;
                case GamePhase.Outro:
                    if (gameOverType.Equals(GameOverType.HappyEnding))
                    {
                        AudioPlayer.GetInstance().PlayMusic(2);
                        if (Main.gameLevel < Main.MAX_GAME_LEVEL)
                        {
                            AudioPlayer.GetInstance().PlayMusic(1, true, 6.0f);
                        }
                    }
                    else {
                        AudioPlayer.GetInstance().PlayMusic(3);
                        AudioPlayer.GetInstance().PlayMusic(0, true, 4.0f);
                    }
                    break;
            }
        }
